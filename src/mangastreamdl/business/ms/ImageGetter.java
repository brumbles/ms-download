...
public class ImageGetter extends DefaultHandler
{
    ...
    if (localName.equals("style")) {
	    ...
	    divStyle = attributes.getValue("style");
	    if (first && divStyle != null && divStyle.matches(".*width:.*;height:.*")){
		ip.setWidthAndHeight(parseWidthAndHeight(divStyle));
		first = false;
		inInline = true;
	    }
	    else if (divStyle != null && inInline){
		parseInlineStyle(divStyle);
		inline = true;
	    }
	    if (id != null && ip.hasID(id)) {
		this.m_id = id;
		this.found = true;
	    }
	}*/
    ...
    private int[] parseWidthAndHeight(String s)
    {
        ...
        {
            if (sarr[i].contains("width"))
            {
                ret[0] = parseNumber(sarr[i]);
            }
            else if (sarr[i].contains("height"))
            {
                ret[1] = parseNumber(sarr[i]);
            }
        }
        return ret;
    }
    ...
    private void parseID(String line)
    {
        ...
        {
            if (sarr[j].contains("z-index"))
            {
                z_index = parseNumber(sarr[j]);
            }
            else if (sarr[j].contains("width"))
            {
                width = parseNumber(sarr[j]);
            }
            else if (sarr[j].contains("height"))
            {
                height = parseNumber(sarr[j]);
            }
            else if (sarr[j].contains("top"))
            {
                top = parseNumber(sarr[j]);
            }
            else if (sarr[j].contains("left"))
            {
                left = parseNumber(sarr[j]);
            }
        }
